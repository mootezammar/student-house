...
const AddHouse = () => {
  const { getAxios, getProperties } = useAppContext();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("garages", form.garages || "0");
      ...
      const { data } = await ax.post("/api/properties", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (data.success) {
        setSubmitted(true);
        await getProperties(); // refresh properties
    ...
    return (
      <div className="flex-1 m-1 sm:m-3 h-[97vh] bg-white shadow-sm ring-1 ring-slate-900/5 rounded-xl flexCenter flex-col gap-4">
        ...
        <p className="regular-14 text-gray-400 text-center max-w-xs">
          <span className="font-semibold text-gray-600">{form.title}</span> has been successfully added.
        </p>
        <button
    ...
    <div className="flex-1 md:px-8 py-6 m-1 sm:m-3 h-[97vh] overflow-y-scroll bg-white shadow-sm ring-1 ring-slate-900/5 rounded-xl">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8 max-w-3xl">
        ...
        <div>
          ...
          <p className="regular-12 text-gray-400 mb-3">Upload 4 images. First image will be the cover.</p>
                ...
                {images[index] ? (
                  <div className="relative">
                    <img
                      ...
                      alt={`preview-${index}`}
                      className={`w-full h-28 object-cover rounded-xl ring-2 ${index === 0 ? "ring-secondary" : "ring-slate-900/10"}`}
                    />
                    {index === 0 && (
                      <span className="absolute top-2 left-2 bg-secondary text-white text-xs px-2 py-0.5 rounded-full">Cover</span>
                    )}
                    <label className="absolute inset-0 cursor-pointer opacity-0 group-hover:opacity-100 bg-black/40 rounded-xl flexCenter transition-opacity">
                      ...
                      <input type="file" accept="image/*" onChange={(e) => handleImage(e, index)} className="hidden" />
                    </label>
                    <button
                      type="button"
                  ...
                  <label className={`flex flex-col items-center justify-center w-full h-28 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
                    index === 0 ? "border-secondary/50 bg-secondary/5 hover:bg-secondary/10" : "border-gray-200 bg-gray-50 hover:bg-gray-100"
                  }`}>
                    <span className="text-2xl mb-1">{index === 0 ? "🖼️" : "➕"}</span>
                    <p className="regular-12 text-gray-400 text-center px-1">{index === 0 ? "Cover photo" : `Photo ${index + 1}`}</p>
                    <input type="file" accept="image/*" onChange={(e) => handleImage(e, index)} className="hidden" />
                  </label>
                )}
              </div>
          ...
          {errors.images && <p className="text-red-400 text-xs mt-2">{errors.images}</p>}
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <input name="title" value={form.title} onChange={handleChange} placeholder="Ex: Modern Studio near University" className={inputClass("title")} />
              {errors.title && <p className="text-red-400 text-xs mt-1">{errors.title}</p>}
            </div>
            <div>
              <label className="medium-13 text-gray-500 mb-1 block">Description *</label>
              <textarea name="description" value={form.description} onChange={handleChange} rows={4} placeholder="Describe the property..." className={`${inputClass("description")} resize-none`} />
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="rent" type="number" min="0" value={form.rent} onChange={handleChange} placeholder="Ex: 400" className={inputClass("rent")} />
        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <input name="address" value={form.address} onChange={handleChange} placeholder="Ex: 12 Rue de la République" className={inputClass("address")} />
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="country" value={form.country} className={inputClass("country") + " opacity-60 cursor-not-allowed"} disabled />
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={name}>
                <label className="medium-13 text-gray-500 mb-1 block">{icon} {label} {required && "*"}</label>
                <input name={name} type="number" min="0" value={form[name]} onChange={handleChange} placeholder="0" className={inputClass(name)} />
        ...
        <div>
          ...
          {form.amenities.length > 0 && (
            <p className="regular-12 text-secondary mt-2">{form.amenities.length} selected</p>
          )}
        </div>

        {/* Submit */}
        <div className="flex gap-3 pb-6">
          <button type="button" onClick={resetForm} className="flex-1 py-3 rounded-xl border-2 border-gray-200 medium-14 text-gray-500 hover:border-secondary transition-colors">
            Reset
          </button>
          <button type="submit" disabled={loading} className="flex-[2] btn-dark py-3 rounded-xl medium-14">
